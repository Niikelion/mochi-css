...
## Vanilla / Stitches consumers

This section covers changes that affect users of `@mochi-css/vanilla`, `@mochi-css/stitches`, and `@mochi-css/react` — i.e.,
code that writes styles and uses styled components.

### `@mochi-css/react`

#### Moved

`@mochi-css/react` has been moved to `@mochi-css/vanilla-react` package.
Old name has been deprecated.

#### Type improvement: `MochiCSS<V[K]>` in `styled()` spread params

The variadic spread params of `styled()` now accept `MochiCSS<V[K]>` instead of unparameterized `MochiCSS`.
This preserves variant type information when a typed `MochiCSS` instance is passed as a base style.

This is a non-breaking improvement for runtime behavior.
TypeScript may surface new errors if you were passing a `MochiCSS<SomeVariants>` where the types now need to align with the component's own variant type `V[K]`.
Update the type parameter or cast as needed.

---

## Core API consumers

This section covers changes that affect code consuming `@mochi-css/builder`, `@mochi-css/config`, `@mochi-css/plugins`,
or any other integration-level API — i.e., custom tooling, plugin authors, and framework integrations.
