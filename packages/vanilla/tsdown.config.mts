...
interface CssFeature { name: string; syntax?: string }
interface CssData { properties: CssFeature[]; types: CssFeature[] }
...
function findDefaultUnit(
    ...
    for (const t of types)
        if (t in numericTypeUnits)
            return numericTypeUnits[t as keyof typeof numericTypeUnits]
...
function generateFileContent(units: Record<string, string>) {
    const entries = Object.entries(units)
        ...
        .join(`\n${" ".repeat(3 * 4)}`)
    return /* language=typescript */ dedent`
