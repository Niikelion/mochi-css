        ...
        init(
            ...
            const styleExprs = registry.fileCache(
                ...
                (file): StyleExprResult => {
                    ...
                    const { mergedExtractorIds, parentCalls } =
                        derivedInst.derived.for(file).get();

                    const styleExpressions = new Set<SWC.Expression>();
                    const extractedExpressions = new Map<
                        StyleExtractor,
                        SWC.Expression[]
                    >();
                    const extractedCallExpressions = new Map<
                        StyleExtractor,
                        SWC.CallExpression[]
                    >();
                            ...
                            callExpression(node, { descend }) {
                                if (node.callee.type === "Identifier") {
                                    ...
                                    const styleExtractor =
                                        mergedExtractorIds.get(calleeRef);
                                    if (styleExtractor) {
                                        const staticArgs =
                                            styleExtractor.extractStaticArgs(
                                                node,
                                            );
                                        staticArgs.forEach((style) =>
                                            styleExpressions.add(style),
                                        );

                                        if (!parentCalls.has(node)) {
                                            const existing =
                                                extractedExpressions.get(
                                                    styleExtractor,
                                                );
                                            if (existing) {
                                                existing.push(...staticArgs);
                                            } else {
                                                extractedExpressions.set(
                                                    styleExtractor,
                                                    [...staticArgs],
                                                );
                                            }
                                            if (staticArgs.length > 0) {
                                                const existingCalls =
                                                    extractedCallExpressions.get(
                                                        styleExtractor,
                                                    );
                                                if (existingCalls) {
                                                    existingCalls.push(node);
                                                } else {
                                                    extractedCallExpressions.set(
                                                        styleExtractor,
                                                        [node],
                                                    );
                                                }
