...
export function makeImportSpecStage(
    extractors: StyleExtractor[],
): StageDefinition<[], ImportSpecStageOut> {
    const extractorLookup: ExtractorLookup = new Map();
    for (const extractor of extractors) {
        getOrInsert(extractorLookup, extractor.importPath, () => new Map()).set(
    ...
    const stage = defineStage({
        ...
        init(registry: CacheRegistry): ImportSpecStageOut {
            const fileData: FileInput<FileData> =
                registry.fileInput<FileData>();
            ...
            const importSpecs = registry.fileCache(
                ...
                (file): RefMap<StyleExtractor> => {
                    ...
                    for (const item of data.ast.body) {
                        ...
                        const possibleExtractors = extractorLookup.get(
                            item.source.value,
                        );
