...
import { transformCallIds } from "./styledIdTransform";
...
describe("transformCallIds", () => {
    it("injects an s- id into a simple styled call", () => {
        const source = `const Button = styled('button', { color: 'red' })`;
        const result = transformCallIds(source, FILE, STYLED);
        expect(result).toContain("'s-");
        expect(result).toMatch(
            /styled\('button', \{ color: 'red' }, 's-[0-9A-Za-z_-]+'\)/,
        );
    });

    it("is idempotent — does not inject if last arg is already s- string", () => {
        const source = `const Button = styled('button', { color: 'red' })`;
        const once = transformCallIds(source, FILE, STYLED);
        const twice = transformCallIds(once, FILE, STYLED);
    ...
    it("generates different IDs for different variable names in the same file", () => {
        ...
        `;
        const result = transformCallIds(source, FILE, STYLED);
        const ids = [...result.matchAll(/'s-([0-9A-Za-z_-]+)'/g)].map(
            (m) => m[0],
        );
    ...
    it("generates different IDs for the same var name in different files", () => {
        ...
        const r1 = transformCallIds(source, "/src/A.tsx", STYLED);
        const r2 = transformCallIds(source, "/src/B.tsx", STYLED);
        const id1 = /'s-([0-9A-Za-z_-]+)'/.exec(r1)?.[0];
        const id2 = /'s-([0-9A-Za-z_-]+)'/.exec(r2)?.[0];
        expect(id1).not.toBe(id2);
    });

    it("returns source unchanged when no symbol name appears in source", () => {
        const source = `const Button = css({ color: 'red' })`;
        expect(transformCallIds(source, FILE, STYLED)).toBe(source);
    });
    ...
    it("handles exported variable declarations", () => {
        ...
        const result = transformCallIds(source, FILE, STYLED);
        expect(result).toMatch(
            /styled\('section', \{ padding: 16 }, 's-[0-9A-Za-z_-]+'\)/,
        );
    });
    ...
    it("uses index-based ID for inline expression statement styled calls", () => {
        ...
        const result = transformCallIds(source, FILE, STYLED);
        expect(result).toMatch(
            /styled\('div', \{ margin: 0 }, 's-[0-9A-Za-z_-]+'\)/,
        );
    });
    ...
    it("does not inject into function declaration following a multiline styled call", () => {
        ...
        const result = transformCallIds(
            source,
            "/project/src/Subpage.tsx",
            STYLED,
        );
    ...
    it("works with a custom symbol name", () => {
        const source = `const Foo = myStyled('div', { color: 'blue' })`;
        const result = transformCallIds(source, FILE, new Set(["myStyled"]));
        expect(result).toMatch(
            /myStyled\('div', \{ color: 'blue' }, 's-[0-9A-Za-z_-]+'\)/,
        );
    });

    it("injects for all matching symbol names when multiple are provided", () => {
        const source = `
            const A = styled('div', { color: 'red' })
            const B = myStyled('span', { color: 'blue' })
        `;
        const result = transformCallIds(
            source,
            FILE,
            new Set(["styled", "myStyled"]),
        );
        const ids = [...result.matchAll(/'s-([0-9A-Za-z_-]+)'/g)];
