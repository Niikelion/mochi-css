...
export function collectCallsBySymbol(
    ast: SWC.Module,
 ...
 */
export function transformCallIds(
    source: string,
    filePath: string,
    symbolNames: Set<string>,
): string {
    ...
    const toInject = calls.filter((entry) => {
        ...
        return !(
            last?.expression.type === "StringLiteral" &&
            STABLE_ID_RE.test(last.expression.value)
        );
    });
    ...
    const sortedAsc = [...toInject].sort(
        (a, b) => a.call.span.start - b.call.span.start,
    );
    ...
    for (const entry of sortedAsc) {
        const symbolName =
            entry.call.callee.type === "Identifier"
                ...
                : entry.call.callee.type === "MemberExpression" &&
                    entry.call.callee.property.type === "Identifier"
        ...
        if (callsWithOffset.length === 0) {
            ...
            const symbolGlobal =
                callee.type === "Identifier"
                    ? callee.span.start
                    : (callee as SWC.MemberExpression).property.span.start;
    ...
    callsWithOffset.forEach(({ entry, offset }, i) => {
        ...
        if (offset < 0 || offset >= result.length || result[offset] !== ")")
            return;
