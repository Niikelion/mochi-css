...
function findDependencyCssCallGroups(
    ...
    const declaratorToGroup = new Map<SWC.VariableDeclarator, DependencyCssCallGroup>()
    ...
    for (const binding of info.usedBindings) {
        ...
        const trackedCallArgs = callArgs.filter((a) => trackedExpressions.has(a))
...
function makeArgsVarDecl(group: DependencyCssCallGroup): SWC.VariableDeclaration & { ctxt: number } {
    const arrayExpr: SWC.ArrayExpression = {
        type: "ArrayExpression",
        span: emptySpan,
        elements: group.expressions.map((expr) => ({ expression: expr })),
    }
    return makeVarDecl("const", [makeVarDeclarator(makeIdentifier(group.varName, 0), arrayExpr)])
}

function makeSpreadCallExpr(group: DependencyCssCallGroup): SWC.CallExpression & { ctxt: number } {
    return {
        type: "CallExpression",
        span: emptySpan,
        ctxt: 0,
        callee: group.callInit.callee,
        arguments: [{ spread: emptySpan, expression: makeIdentifier(group.varName, 0) }],
    }
}
...
export function extractRelevantSymbols(index: ProjectIndex): Record<string, string | null> {
    return Object.fromEntries(
        index.files.map(([filePath, info]) => {
            ...
            const { expressionToGroup, declaratorToGroup } = findDependencyCssCallGroups(info, counter)
            ...
            for (const item of usedItems) {
                ...
                if (minimalItem.type === "VariableDeclaration") {
                    ...
                    if (itemGroups.length > 0) {
                        ...
                        const modifiedDeclarators = minimalItem.declarations.map((d) => {
                            const group = declaratorToGroup.get(d)
                            if (!group) return d
                            return makeVarDeclarator(d.id, makeSpreadCallExpr(group))
                        })
                        moduleBody.push({ ...minimalItem, declarations: modifiedDeclarators })
                    } else {
            ...
            const callStatements = generateCallStatements(filePath, info, derivedLookup, expressionToGroup)
