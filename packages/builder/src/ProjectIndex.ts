    ...
    for (const item of ast.body) {
        switch (item.type) {
            case "ImportDeclaration": {
                ...
                const sourcePath = isLocal ? resolveImport(filePath, item.source.value) : null
                ...
                for (const specifier of item.specifiers) {
                    ...
                    const sourceName =
                        specifier.type === "ImportSpecifier" ? (specifier.imported?.value ?? ref.name) : ref.name // default import
            ...
            case "ExportDeclaration": {
                ...
                const identifier =
                    decl.type === "FunctionDeclaration" || decl.type === "ClassDeclaration" ? decl.identifier : null
                if (!identifier) break
...
export class ProjectIndex {
            ...
            for (const fileInfo of this.filesInfo.values()) {
                for (const localImport of fileInfo.localImports.values()) {
                    ...
                    const importedBinding: DerivedExtractorBinding = {
                        ...
                        localIdentifier:
                            fileInfo.moduleBindings.get(localImport.localRef)?.identifier ??
    ...
    private scanForDerivedCalls(fileInfo: FileInfo, ref: Ref, binding: DerivedExtractorBinding): void {
        let found = false
        visit.module(
            fileInfo.ast,
            {
            ...
            switch (specifier.type) {
                case "ImportSpecifier": {
                    return {
                        source,
                        ref,
                        sourceName: specifier.imported?.value ?? ref.name,
                        isNamespace: false,
